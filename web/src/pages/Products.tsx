...
export default function Products({ onSelect }: ProductsProps) {
  ...
  const handleOpenDiscountModal = async (product: Product, e: React.MouseEvent) => {
    e.stopPropagation() // Prevent triggering selection if card is clicked
  ...
  const content = (
    <div style={onSelect ? { padding: '0' } : { padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <h1 style={{ marginBottom: '2rem' }}>{onSelect ? 'Məhsul Seçimi' : 'Məhsullar'}</h1>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: onSelect ? '#f9f9f9' : 'white',
                    transition: 'transform 0.2s',
                    transform: onSelect ? 'scale(1)' : 'none'
                  }}
                  onMouseEnter={(e) => {
                    if (onSelect) {
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #ddd', textAlign: 'left' }}>
                    ...
                    <th style={{ padding: '0.5rem', textAlign: 'right' }}>Əməliyyat</th>
                  </tr>
                </thead>
                <tbody>
                  {discounts.length === 0 ? (
                    <tr>
                      <td colSpan={4} style={{ padding: '1rem', textAlign: 'center', color: '#666' }}>Endirim yoxdur</td>
                    </tr>
                  ) : (
                    discounts.map(d => (
                      <tr key={d.id} style={{ borderBottom: '1px solid #eee' }}>
                        <td style={{ padding: '0.5rem', fontWeight: 'bold', color: '#28a745' }}>{d.percentage}%</td>
                        <td style={{ padding: '0.5rem' }}>{new Date(d.start_date).toLocaleDateString()}</td>
                        <td style={{ padding: '0.5rem' }}>{new Date(d.end_date).toLocaleDateString()}</td>
  ...
  if (onSelect) {
    return content // Selection mode: No Layout, just content
  }
