...
const FILTER_COMPONENTS: FilterComponentConfig[] = [
  ...
  { id: 'customer_id', label: 'Təchizatçı', type: 'partner', partnerType: 'SUPPLIER' },
...
const defaultColumns: ColumnConfig[] = [
  ...
  { id: 'invoice_number', label: '№', visible: true, width: 140, order: 2 },
  {
    ...
    width: 220,
    order: 3,
    render: (_value: any, row: any) => row.supplier_name || ''
  },
...
export function AlisQaimeleriContent() {
  ...
  useEffect(() => {
    try {
      const savedRules = localStorage.getItem('filter-applied-filter-alis')
      if (savedRules) {
        ...
        return appliedFilters.every(rule => {
          ...
          if (rule.component === 'customer_id') {
            ...
            if (isTextOp || isSubjectString) {
              // Compare Names
              const supplier = suppliers.find(s => s.id === item.customer_id)
              itemValue = supplier?.name || ''
  ...
  const openModalForInvoice = async (invoiceId: number | null = null) => {
    try {
      ...
      let supplierObj = fullInvoice?.customers || null
      if (fullInvoice?.customer_id && !supplierObj) {
        // Try to find in loaded suppliers
        supplierObj = (suppliers.find(s => s.id === fullInvoice!.customer_id) as any) || null

        // If still not found, fetch it
        if (!supplierObj) {
          try {
            console.log('[Alis.tsx] Supplier object missing in invoice, fetching...', fullInvoice.customer_id)
            const fetched = await customersAPI.getById(fullInvoice.customer_id.toString())
            if (fetched) {
              supplierObj = fetched
              // Also update local suppliers list if needed? Maybe not to avoid side effects
            }
          } catch (e) {
            console.warn('[Alis.tsx] Failed to fetch supplier details', e)
          }
        }
      }
      ...
      const newModal: ModalData = {
        ...
        data: {
          ...
          selectedSupplier: supplierObj as any,
          invoiceItems: items,
  ...
  const tableData = filteredInvoices.map(invoice => ({
    ...
    supplier_name: invoice.customers?.name || suppliers.find(s => s.id === invoice.customer_id)?.name || '-',
        ...
        onPeriodChange={(period: any) => {
          ...
          if (period.startDate || period.endDate) {
            newRules.push({
              ...
              condition: 'between', // This condition is just a placeholder or needs to be handled if I strictly check conditions
              value: period
            } as any)
          }
          setAppliedFilters(newRules)
          localStorage.setItem('filter-applied-filter-alis', JSON.stringify(newRules))
        }}
        onFilter={() => {
          useWindowStore.getState().openPageWindow(
            'advanced-filter-alis',
            'Filtrlər',
            '🔍',
            <AdvancedFilterModal
              onClose={() => useWindowStore.getState().closePageWindow('advanced-filter-alis')}
              toolbarId="filter-alis"
              onApply={(rules) => {
                setAppliedFilters(rules)
                localStorage.setItem('filter-applied-filter-alis', JSON.stringify(rules))
                useWindowStore.getState().closePageWindow('advanced-filter-alis')
              }}
