...
export default function SaleInvoicesScreen({ navigation }) {
  ...
  const renderInvoice = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('SaleInvoiceDetail', { invoiceId: item.id })}
    >
  ...
  return (
    <LinearGradient colors={['#667eea', '#764ba2']} style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Satış Qaimələri</Text>
      </View>
      ...
      {invoices.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>Satış qaiməsi yoxdur</Text>
        </View>
      ) : (
        <FlatList
          data={invoices}
