...
export default function SaleInvoicesScreen({ navigation }) {
  ...
  const loadInvoices = async () => {
    try {
      setLoading(true);
      const result = await saleService.getInvoices();
      if (result.success) {
  ...
  const handleEdit = () => {
    ...
    navigation.navigate('ProductSale', { 
    ...
    Alert.alert(
      ...
      [
        { text: 'Ləğv et', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const result = await saleService.deleteInvoice(selectedInvoice.id);
              if (result.success) {
  ...
  const renderTableRow = (item, index) => (
    <TouchableOpacity
      ...
      onLongPress={() => navigation.navigate('SaleInvoiceDetail', { invoiceId: item.id })}
    >
      ...
      <Text style={[styles.tableCell, styles.cellCustomer]} numberOfLines={1}>
        {item.customer_name || '-'}
      </Text>
      ...
      <View style={styles.header}>
        <View style={styles.headerButtons}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ProductSale', { isEdit: false })}
          >
      ...
      {showSearch && (
        <View style={styles.searchContainer}>
          <TextInput
            ...
            placeholder="Axtarış (qaimə nömrəsi, müştəri)..."
        ...
        {filteredInvoices.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {searchQuery ? 'Axtarış nəticəsi tapılmadı' : 'Satış qaiməsi yoxdur'}
            </Text>
          </View>
        ) : (
          <ScrollView 
          ...
          >
            <View style={styles.tableWrapper}>
              ...
              <View style={styles.tableHeader}>
                ...
                <TouchableOpacity 
                  style={[styles.tableHeaderCell, styles.cellCustomer]}
                  onPress={() => handleSort('customer_name')}
                >
                  <Text style={styles.tableHeaderText}>Müştəri</Text>
                  <Ionicons 
                    name={getSortIcon('customer_name')} 
                    size={16} 
                    color={sortColumn === 'customer_name' ? '#667eea' : '#999'} 
                  />
                </TouchableOpacity>
                <TouchableOpacity 
